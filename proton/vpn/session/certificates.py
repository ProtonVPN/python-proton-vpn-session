...
class Asn1BerDecoder:
    ...
    def __get_asn1_ber_len(cls, raw: bytes) -> typing.Tuple[int, int]:
        ...
        if raw[1] & 0x80 == 0:
            ...
            data_len = 0
            for b in raw[2:2+length_of_length]:
    ...
    def _transform_value_to_int_no_len_check(cls, raw: bytes) -> typing.Tuple[int, int]:
        ...
        data_len, pos_data = cls.__get_asn1_ber_len(raw)
        val = 0
        for b in raw[pos_data:pos_data+data_len]:
            val = val * 256 + b
        return val, (pos_data + data_len)

    @classmethod
    def transform_value_to_int(cls, raw: bytes) -> int:
    ...
    def _transform_value_to_sequence_no_len_check(cls, raw: bytes) -> typing.Tuple[list, int]:
        ...
        if raw[0] not in (cls._TYPE_SEQUENCE, cls._TYPE_SEQUENCE_OF):
            raise ValueError("Not a sequence : {}".format(raw))
        data_len, pos_data = cls.__get_asn1_ber_len(raw)
        if data_len == 0 and raw[1] == 0x80:
            indefinite_len = True
        else:
        ...
        while True:
            if indefinite_len:
                ...
                if raw[current_pos] == 0 and raw[current_pos + 1] == 0:
                    ...
                    if current_pos != len(raw):
                        raise ValueError("wrong extension length : {} , indefinite len ending at position {}, expected {}".format(raw, data_len, len(raw)))
                    break
            else:
                if current_pos == pos_data + data_len:
                    break
                elif current_pos > pos_data + data_len:
                    raise IndexError(f"Error parsing data : current_pos = {current_pos} / pos_data = {pos_data} / data_len = {data_len} / raw = {raw}")
            if raw[current_pos] == cls._TYPE_INTEGER:
                ...
                raise NotImplementedError(f"Unknown type found : 0x{raw[current_pos]:02x} at position {current_pos} in raw = {raw}")
        return decoded_list, current_pos

    @classmethod
    def transform_value_to_sequence(cls, raw: bytes) -> list:
...
class Extension:
    ...
    @property
    def value_as_sequence(self) -> list:
...
class Certificate:
    ...
    def __init__(self, cert_pem: typing.Union[bytes, str] = None, cert_der: bytes = None):
        ...
        if len(cert_input) > 1:
            raise ValueError("Not possible to provide multiple cert format. Provided formats = {}".format("/".join([x_type for _, x_type in cert_input])))
        ...
        if (int(crypto_major) < 3 or
            ...
            backend_x509 = cryptography.hazmat.backends.default_backend()  # backend is required if library < 3.1
    ...
    @property
    def proton_fingerprint(self) -> str:
    ...
    @property
    def validity_date(self) -> datetime.datetime:
    ...
    def get_as_pem(self) -> str:
        return self._cert.public_bytes(Encoding.PEM).decode("ascii")

    @property
    def proton_extensions(self) -> typing.Dict[ExtName, Extension]:
        extensions = {}
