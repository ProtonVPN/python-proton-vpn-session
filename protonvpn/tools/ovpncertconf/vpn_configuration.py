import os
import tempfile
from abc import ABC, abstractmethod

import jinja2
from jinja2 import Environment, BaseLoader
from enum import Enum

TEMPLATES='templates'
OPENVPN_TEMPLATE='openvpn_cert_template.j2'
PROTON_XDG_CACHE_HOME='/tmp'

OVPN_CERT_TEMPLATE="""
client
dev tun
proto {{ openvpn_protocol|lower }}

{% for ip in serverlist %}
{%- for port in openvpn_ports -%}
remote {{ ip }} {{ port }}
{% endfor %}
{% endfor -%}

remote-random
resolv-retry infinite
nobind
cipher AES-256-CBC
auth SHA512
comp-lzo no
verb 3

tun-mtu 1500
tun-mtu-extra 32
mssfix 1450
persist-key
persist-tun

reneg-sec 0

remote-cert-tls server
pull
fast-io

{%- if ipv6_disabled %}

# Remove IPv6 related configurations
pull-filter ignore "ifconfig-ipv6"
pull-filter ignore "route-ipv6"
{%- endif %}

{%- if split %}

# Split Tunneling
{%- for ip_nm_pair in ip_nm_pairs %}
route {{ ip_nm_pair.ip }} {{ ip_nm_pair.nm }} net_gateway
{%- endfor %}
{%- endif %}
<ca>
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
</ca>
key-direction 1
<tls-auth>
-----BEGIN OpenVPN Static key V1-----
6acef03f62675b4b1bbd03e53b187727
423cea742242106cb2916a8a4c829756
3d22c7e5cef430b1103c6f66eb1fc5b3
75a672f158e2e2e936c3faa48b035a6d
e17beaac23b5f03b10b868d53d03521d
8ba115059da777a60cbfd7b2c9c57472
78a15b8f6e68a3ef7fd583ec9f398c8b
d4735dab40cbd1e3c62a822e97489186
c30a0b48c7c38ea32ceb056d3fa5a710
e10ccc7a0ddb363b08c3d2777a3395e1
0c0b6080f56309192ab5aacd4b45f55d
a61fc77af39bd81a19218a79762c3386
2df55785075f37d8c71dc8a42097ee43
344739a0dd48d03025b0450cf1fb5e8c
aeb893d9a96d1f15519bb3c4dcb40ee3
16672ea16c012664f8a9f11255518deb
-----END OpenVPN Static key V1-----
</tls-auth>
<cert>
{{cert}}
</cert>
<key>
{{priv_key}}
</key>
"""


class ProtocolEnum(Enum):
    TCP = "tcp"
    UDP = "udp"
    IKEV2 = "ikev2"
    WIREGUARD = "wireguard"

class VPNConfiguration(ABC):
    """VPNConfiguration class.

    Generates VPN configuration that can be used to
    import via NM tool.
    """

    def __init__(self, physical_server, vpnaccount):
        self._physical_server = physical_server
        self._vpnaccount = vpnaccount
        self._configfile = None

    @classmethod
    def factory(cls, protocol : ProtocolEnum, physical_server, vpnaccount, *a, **kw):
        if not isinstance(protocol, ProtocolEnum):
            err_msg = "Incorrect object type, "\
                "ProtocolEnum is expected but got {} instead".format(
                    type(protocol)
                )
            raise TypeError(err_msg)

        protocol_dict = {
            ProtocolEnum.TCP : VPNConfigurationOpenVPNTCP,
            ProtocolEnum.UDP : VPNConfigurationOpenVPNUDP,
            ProtocolEnum.IKEV2: VPNConfigurationStrongSwan,
            ProtocolEnum.WIREGUARD : VPNConfigurationWireguard,
        }

        if protocol not in protocol_dict:
            raise ValueError

        return protocol_dict[protocol](physical_server, vpnaccount, *a, **kw)

    @abstractmethod
    def generate(self):
        pass

    @property
    @abstractmethod
    def config_extn(self):
        """Config file extension"""
        pass

    def __enter__(self):
        # We create the configuration file when we enter,
        # and delete it when we exit.
        # This is a race free way of having temporary files.
        if self._configfile is None:
            self.__delete_existing_ovpn_configuration()
            self._configfile = tempfile.NamedTemporaryFile(
                dir='/tmp', delete=False,
                prefix='ProtonVPN-', suffix=self.config_extn, mode='w'
            )
            self._configfile.write(self.generate())
            self._configfile.close()
            self._configfile_enter_level = 0

        self._configfile_enter_level += 1

        with open(self._configfile.name,'r') as f:
            return f.read()

    def __exit__(self, exc_type, exc_val, exc_tb):
        if self._configfile is None:
            return

        self._configfile_enter_level -= 1
        if self._configfile_enter_level == 0:
            os.unlink(self._configfile.name)
            self._configfile = None

    def __delete_existing_ovpn_configuration(self):
        for file in os.listdir(PROTON_XDG_CACHE_HOME):
            if file.endswith(".ovpn"):
                os.remove(
                    os.path.join(PROTON_XDG_CACHE_HOME, file)
                )


class VPNConfigurationOpenVPN(VPNConfiguration):
    """VPNConfiguation class.

    Generates VPN configuration that can be used to
    import via NM tool.
    """

    @property
    def config_extn(self):
        return '.ovpn'

    @property
    @abstractmethod
    def openvpn_protocol_name(self):
        """Return protocol name for use in OpenVPN config"""
        pass

    def generate(self):
        """Method that generates a vpn certificate.

        Returns:
            string: configuration file
        """

        j2_values = {
            "openvpn_protocol": self.openvpn_protocol_name,
            "serverlist": [self._physical_server.entry_ip],
            "openvpn_ports": self._physical_server.ports,
            "cert": self._vpnaccount.get_client_api_pem_certificate(),
            "priv_key": self._vpnaccount.get_client_private_openvpn_key()
        }

        #j2 = Environment(loader=FileSystemLoader(TEMPLATES))
        #from jinja2 import Environment, BaseLoader

        template = Environment(loader=BaseLoader).from_string(OVPN_CERT_TEMPLATE)

        #template = j2.get_template(OPENVPN_TEMPLATE)

        try:
            return template.render(j2_values)
        except jinja2.exceptions.TemplateNotFound as e:
            raise jinja2.exceptions.TemplateNotFound(e)
        except Exception as e:
            pass


class VPNConfigurationOpenVPNTCP(VPNConfigurationOpenVPN):
    protocol = ProtocolEnum.TCP

    @property
    def openvpn_protocol_name(self):
        return "tcp"


class VPNConfigurationOpenVPNUDP(VPNConfigurationOpenVPN):
    protocol = ProtocolEnum.UDP

    @property
    def openvpn_protocol_name(self):
        return "udp"


class VPNConfigurationStrongSwan(VPNConfiguration):
    protocol = ProtocolEnum.IKEV2
    pass


class VPNConfigurationWireguard(VPNConfiguration):
    protocol = ProtocolEnum.WIREGUARD
    pass
